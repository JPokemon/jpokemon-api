...
class JavadocRestCompiler(object):
    ...
    def __output_doc(self, documented):
        ...
        if doc.author:
            output.add_line(':codeauthor: %s' % (self.__html_to_rst(doc.author),))
        ...
        if doc.tags.get('note'):
            output.clear()
            for note in doc.tags['note']:
                output.add_line('.. note::')
                output.add_line('   %s' % (self.__html_to_rst(note),))
